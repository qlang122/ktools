...
namespace Krane {
    ...
    Magick::Image KBuild::Symbol::Frame::getImage() const {
        ...
        float_type w0, h0;
        {
            ...
            try {
                ...
                cerr << "When cropping " << int(w0) << "x" << int(h0) << " atlas "
                     << parent->parent->atlases[getAtlasIdx()].first << " to " << geo.width() << "x" << geo.height()
                     << "+" << geo.xOff() << "+" << geo.yOff() << " to select image " << getUnixPath() << endl;
            }
        }
        ...
        try {
            ...
            cerr << w.what() << endl;
            cerr << "When reszing " << img.columns() << "x" << img.rows() << " image " << getUnixPath() << " to "
                 << scaling_geo.width() << "x" << scaling_geo.height() << endl;
        }
