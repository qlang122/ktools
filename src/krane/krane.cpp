    ...
    for (iter_t it = inputs.begin(); it != inputs.end(); ++it) {
        ...
        if (magic_number == KBuildFile::MAGIC_NUMBER) {
            ...
            if (newbild == NULL) {
                ...
                delete bild;
                throw KToolsError(
                        "Multiple builds found in the input. Restrict the choice by specifying a build name.");
            }
...
static void configure_bank_collection(KAnimBankCollection &banks) {
    ...
    if (options::allowed_banks.size() > 0) {
        ...
        for (options::allowed_banks_t::const_iterator it = options::allowed_banks.begin();
