...
namespace {
    ...
    struct BuildSymbolMetadata : public std::vector<BuildSymbolFrameMetadata> {
        ...
        uint32_t getFileId(uint32_t build_framenum) const {
            if (symbol == NULL) {
                throw logic_error(
                        "Attempt to access BuildSymbolMetadata object without prior setting of the symbol pointer.");
            }
    ...
    struct AnimationFrameExporterState : public GenericState {
        ...
        AnimationFrameExporterState(uint32_t _animation_frame_id, int max_z_index) : animation_frame_id(
                _animation_frame_id), object_ref_id(0), z_index(max_z_index) {
    ...
    public:
        ...
        void initialize(const KAnim::Frame::Element &elem, uint32_t _start_time) {
            if (M != NULL) {
                throw logic_error(("Reinitializing AnimationSymbolFrameMetadata for frame element " + elem.getName() +
                                   ".").c_str());
            }
            M = &elem.getMatrix();
    ...
    public:
         ...
         */
        void initialize(xml_node animation, uint32_t &current_timeline_id, uint32_t start_time,
    ...
    class AnimationMetadata
            : public map<const KAnim::Frame::Element *, std::list<AnimationSymbolMetadata>, ptrLess<KAnim::Frame::Element> > {
        xml_node animation;
            ...
            if (dupeness == 0) {
                ...
                if (first_animsymmeta == NULL) {
                    throw std::logic_error(
                            "first_comers mapped to NULL, but elemhash_mset has a count greater than 0.");
                }
...
static void exportAnimationFrame(xml_node mainline, AnimationExporterState &s, AnimationMetadata &animmeta,
                                 const BuildMetadata &bmeta, const KAnim::Frame &frame);
...
static void
exportAnimation(xml_node entity, AnimationBankExporterState &s, const BuildMetadata &bmeta, const KAnim &A) {
    ...
    for (AnimationMetadata::const_iterator animsymlistit = animmeta.begin();
        ...
        if (symmeta_match == bmeta.end()) {
            throw logic_error(
                    "Program logic state invariant breached: hashes of keys of AnimationMetadata are not a subset of keys of BuildMetadata.");
        }

        const AnimationMetadata::list_type &animsymlist = animsymlistit->second;

        for (AnimationMetadata::list_type::const_iterator animsymit = animsymlist.begin();
...
exportAnimationSymbolTimeline(const BuildSymbolMetadata &symmeta, const AnimationSymbolMetadata &animsymmeta) {
    ...
    int key_id = 0;
    for (AnimationSymbolMetadata::const_iterator animsymframeiter = animsymmeta.begin();
    ...
    if (symbol_badness != nil) {
        cerr << "WARNING: the geometry of animation symbol \"" << animsymmeta.getName() << "\" in animation \""
             << animsymmeta.getAnimationName()
             << "\" is not fully representable in Spriter. Expect lower animation fidelity. (max matrix distance: "
