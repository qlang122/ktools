...
namespace KTools {
    ...
    namespace ImOp {
        ...
        typedef std::unary_function<MagickCore::PixelPacket *, void> basic_pixel_operation_t;
        ...
        typedef operation_t<MagickCore::PixelPacket *> pixel_operation_t;
        ...
        public:
            virtual void call(MagickCore::PixelPacket *p) const {
                using namespace Magick;

                double a = /*1 -*/ double(p->alpha) / QuantumRange;
                if (a <= 0.1) a = 0;
                else if (a >= 1) a = 1;
        ...
        public:
            virtual void call(MagickCore::PixelPacket *p) const {
                ...
                const double a = /*1 -*/ double(p->alpha) / QuantumRange;
                if (a <= 0 || a >= 1) return;
        ...
        public:
            virtual void call(Magick::Image &img) const {
                ...
                {
                    Magick::Quantum *RESTRICT p = view.get(0, 0, w, h);

                    for (size_t i = 0; i < h; i++) {
                        for (size_t j = 0; j < w; j++) {
                            MagickCore::PixelInfo info;
                            MagickCore::GetPixelInfoPixel(img.constImage(), p++, &info);
                            MagickCore::_PixelPacket packet = MagickCore::PixelPacket();
                            packet.red = info.red;
                            packet.green = info.green;
                            packet.blue = info.blue;
                            packet.alpha = info.alpha;
//                            op(&packet);
                        }
                    }
                }
        ...
        */
        ...
        public:
            ...
            virtual void call(Magick::Image &img) const {
                ...
                Image alpha = img;
                alpha.channel(TrueAlphaChannel);
                alpha.negate();
